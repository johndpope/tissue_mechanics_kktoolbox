function exit_flag = prepare_cluster_array_jobs(...
...
for ix = 1:length(sub_filenames)
    ...
    fn_str = sprintf('%s\\%s%s.script',cluster_path, job_name, num2str(ix));
    fid = fopen(fn_str, 'w');
...
%% generate the jobs script call
fn_str = sprintf('%s\\%s.script',cluster_path, sub_filenames{1});
fid_job_list_script = fopen(fn_str, 'w');                  % start a job list script
script_name = sprintf('%s%s$PBS_ARRAY_INDEX.script', run_job_path,job_name );
text_str = sprintf('#!/bin/bash\n#PBS -M khairy@embl.de\n#PBS -m abe\n#PBS -J 1-%s\n%s', num2str(length(sub_filenames)), script_name);
fwrite(fid_job_list_script, text_str, 'char');
fclose(fid_job_list_script);


%% generate the job-array call
fn_str = sprintf('%s\\run_array.script',cluster_path);
fid_job_run_script = fopen(fn_str, 'w');                  % start a script to run the job script
% script_name = sprintf('%s%s.script', run_job_path,job_name );

script_name = sprintf('%s.script', sub_filenames{1});
text_str = sprintf('#!/bin/bash\n/usr/pbs/bin/qsub  -l select=1:ncpus=3,mppmem=6000Mb -q clusterng %s',script_name);
fwrite(fid_job_run_script, text_str, 'char');
fclose(fid_job_run_script);
