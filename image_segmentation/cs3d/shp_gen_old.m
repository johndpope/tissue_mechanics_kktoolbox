function [T, V_vec] = shp_gen(Iw,Idata, L_max, newton_niter, V_max, V_min)
...
T = [];
V_vec = [];
disp(['Number of segmented objects found: ' num2str(max(Iw(:)))]);
%BW = zeros(size(Iw),'uint8');
for t = max(Iw(:)):-1:1
    disp(['Generating object : ' num2str(t)]);
    BW = (Iw==t);BW(BW) = 1;
    L = bwlabeln(BW);
...
% % 
% % 
% % close all;[A, V] = plot_shp_T(T2, 20, 'b');hold on
...
if ~euler_violation,
    ...
    [A, V, h, v, xclks, yclks, zclks, X, C,t,p, Y_LK] = ...
        sh_projection2(gdim,L_max, L_max, L_max, X(:,1)', X(:,2)', X(:,3)', t', p');
