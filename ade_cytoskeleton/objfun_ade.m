function [E,v_red,A, V,AreaDiff, Eb, EADE, E_stretch, E_shear, Estretch1, Estretch2, Estretch3,Eshear1, Eshear2, Eshear3] = ...
...
Estretch1 = k_stretch/2 * sum((ai.*ai + a3 * ai .*ai.*ai +a4 * ai.*ai.*ai.*ai).*F_areas_o);
Estretch2 = k_stretch/2 * sum((a3 * ai .*ai.*ai).*F_areas_o);
Estretch3 = k_stretch/2 * sum((a4 * ai.*ai.*ai.*ai).*F_areas_o);
...
Eshear1 = k_shear *sum((beta).*F_areas_o);
Eshear2 = k_shear *sum((b1 * ai .*beta).*F_areas_o);
Eshear3 = k_shear *sum((b2 * beta.*beta).*F_areas_o);
