function x = single_shape_ade_ms_energy(clks)
%%% calculate the  energy of the shape numerically
%%% using the area difference elasticity model with 
%%  membrane skeleton
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
mxfun = 1e6;gdim = 120;nc = length(clks);L_max = sqrt(nc/3)-1;
%%% Get starting values
disp(' Calculating initial values');
global C HidAi dAi HidAi_mx dAij
global V_e_ix LVE_ix VE_ix tr1_ix tr2_ix VT_ix LVT_ix V_far_ix u v w crossqpr
%%%% prepare initial mesh
% [A_o,V] = shape_properties_calc(clks);
[A_o,V] = plot_sh(clks);


[X, C,x, y, z,  A, V, v, F_areas, h, H, Eb, da] = mesh_gen(gdim, A_o);nvert = length(X);
...
Y_LK  = zeros(N, M);for S = 1:length(L), Y_LK(:,S) = ylk_cos_sin_old(L(S),K(S),p',t')';end;
...
VT_ix = VT(find(VT~=0));
LVT_ix = (find(VT~=0));
[xclks, yclks, zclks]  = get_xyz_clks(clks);
...
v_target = 0.95; A_So = 140;Vo = 4/3*pi*(A_So/4/pi).^(3/2);
V_target = v_target * Vo;    % this is the actual volume of S_o in microns^3
%% for an oblate ellipse we need to obtain a and c
e = 0.311;    % set the ellipticity that gives the desired vo
fac = log((1+e)/(1-e));
a = 1/(2*pi*e+pi*fac*e^2+pi*fac)*(pi*(2*e+fac*e^2+fac)*A_So*e)^(1/2);
c = sqrt((e*a)^2 + a^2);
Vol = 4/3*pi*a^2 * c; A_actual = 2*pi*a^2 + pi * c^2/e*log((1+e)/(1-e));
disp([ a c A_So A_actual Vol Vo Vol/Vo]);
[x y z] = kk_sph2cart(t,p,1);
u = a * x; v = a * y; w = c * z;
% load Lmax10_v95;   % load the elliptical shape (clks) that has a reduced volume of .95
% %%% Calcuate the areas of the triangles of that shape and store these areas in F_areas_o
% fac = 3.348;   % multiplicative factor to get the area to 140
% xclks_ms = xclks_ms * fac;yclks_ms = yclks_ms * fac;zclks_ms = zclks_ms * fac;
% gdim = size(Y_LK,1);gdimsq = gdim.*gdim;
% So = [sum(xclks_ms(ones(gdim,1),:).*Y_LK,2)  sum(yclks_ms(ones(gdim,1),:).*Y_LK,2) sum(zclks_ms(ones(gdim,1),:).*Y_LK,2)];
% u = So(:,1); v = So(:,2); w = So(:,3);
...
X_o = clks;
q = 2/pi;
[E,v_red,A, V,AreaDiff, Eb, EADE, E_stretch, E_shear,Estretch1, Estretch2, Estretch3,Eshear1, Eshear2, Eshear3] = ...
    objfun_ade(X_o, A_o, q,Y_LK, 1,0.00143,0.28, 1);
% [c, ceq] = confuneq_ade(X_o, A_o, q,Y_LK,1, 0.00143, 1);
view(3);zoom(1);%axis(axis);         % Freeze the axes limits
disp([E, Eb, EADE, E_stretch, E_shear, Estretch1, Estretch2, Estretch3, Eshear1, Eshear2, Eshear3]/E);

% % [E,v_red,A, V,AreaDiff, Eb, EADE, E_stretch, E_shear] = ...
% %     objfun_ade(clks, A_o, q,Y_LK,v_o, a_o_bar, lambda_shear,flag)


% % set(gca,'vis','off')% Get rid of the axes
% axis off
% %%%%%%%%%%%%%%%%%%%%%% Phases
% v_o_vec  = [0.64];
% %a_o_bar_vec = 0.00143* ones(size(v_o_vec));
%   a_o_bar_vec = [0.02003];
% %   a_o_bar_vec = 0.02003* ones(size(v_o_vec));
% 
% % a_o_bar_vec = 0.00143* ones(size(v_o_vec));
% X_o = phase_diagram(A_o, q, v_o_vec, a_o_bar_vec, X_o, mxfun, Y_LK,t, p,  flag, 'phase64_MS');%save phase1;
