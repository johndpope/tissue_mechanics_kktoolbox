function ade_ms_precalc_disco(A_o, L_max, gdim, A_ms, v_red_ms, lambda_shear, discoMS);
%
%
...
[x, y, z, X, C, A, V, v, F_areas, h, H, Eb, da] = mesh_gen(gdim, A_o);nvert = length(X);
...
Y_LK_C1  = zeros(length(C), M);for S = 1:length(L), Y_LK_C1(:,S) = ylk_cos_sin(L(S),K(S),p(C(:,1))',t(C(:,1))')';end;
Y_LK_C2  = zeros(length(C), M);for S = 1:length(L), Y_LK_C2(:,S) = ylk_cos_sin(L(S),K(S),p(C(:,2))',t(C(:,2))')';end;
Y_LK_C3  = zeros(length(C), M);for S = 1:length(L), Y_LK_C3(:,S) = ylk_cos_sin(L(S),K(S),p(C(:,3))',t(C(:,3))')';end;
...
clear ig h g2 g1 fmemb ememb edg counter c V_far 
...
%%%%%%%%%%%%%%% Oblate
% [a, c, A_oblate, Vol, v_ell] = ellipsoid_dimensions_gen(A_ms, v_red_ms, .5);
...
[Area_d,V_d,v_red_d,t_d,p_d,X_d,C_d,Y_LK_d]=...
             plot_sh(discoMS, gdim);
x = X_d(:,1);y = X_d(:,2); z = X_d(:,3);a = 1;c  = 1;
u = a * x; v = a * y; w = c * z;
