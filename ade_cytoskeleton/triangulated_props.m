function [A, V, v, F_areas, h, H, wb, da, N, K, k_g, dA, n] = triangulated_props(X, C, plotflag)
%% compute geometrical properties of the triangular mesh shape defined by X, where X is a
%% three-vector of Cartesian coordinates. 
%% Usage: [A, V, v, F_areas, h, H, wb, da, N, K, k_g] = triangulated_props(X, C, plotflag)
%%  Input:
%%          X: the coordinates of the vertices
%%          C: the faces
%%  Output:
%% The routine calculates A, V, v,
...
% %         normals_working = [];
        ...
        % The average area of triangles around the vertex V_a is given by
         dA(ix) = sum(F_areas(r))/3; %% this is correct
       
        if dA(ix)==0,M(ix) = 0;
            %warning('Probable error in area calculation II');
        else
            H(ix) = H(ix)/dA(ix);
    ...
    if nargout>9, K = real(K)./dA(:);k_g =sum(K.*dA(:));end
    h = sum((-M))./A;
    ...
    str = sprintf('Area: %.5f \nVolume: %.5f\nReduced Volume: %.5f\nTotal mean curvature:%.5f\nE(bending): %.5f\nArea Difference: %.5f\n',...
        A, V,v,h,Eb/8/pi,dA2*D);
    if(plotflag), disp(str);end
