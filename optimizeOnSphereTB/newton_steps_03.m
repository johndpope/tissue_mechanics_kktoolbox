function [t,p] = newton_steps_03( t,p,F,maxiter,flag,filename, verbose,face_memb)
...
warning off;
for iter = 1:maxiter,
    if verbose, if~mod(iter,500), disp(iter);end;end
    ...
    u(:) = cos(pi/2-t(:)).*cos(p(:));v(:) = cos(pi/2-t(:)).*sin(p(:));w(:) = sin(pi/2-t(:)); % convert to Cartesian
    ...
    CHG_vec = seps*X;                           %% generate the finite difference values
    ...
    %disp(cond(J*J'));
    %disp(norm(-Areas-(J*J')*dv)/norm(-Areas));
    %%% and take a step
    stepfac = .5;
    X = X + stepfac.*J'*dv;
    ...
    if verbose==2,
        ...
        clf; patch('Vertices',[u v w],'Faces',F,'FaceVertexCData', hsv(size(F,1)),'FaceColor','flat');axis square;view(-276,4);zoom(1);drawnow;
    end
...
if verbose,
    u(:) = cos(pi/2-t(:)).*cos(p(:));v(:) = cos(pi/2-t(:)).*sin(p(:));w(:) = sin(pi/2-t(:));
    clf; patch('Vertices',[u v w],'Faces',F,'FaceVertexCData', hsv(size(F,1)),'FaceColor','flat');axis square;view(-123,-18);zoom(1);drawnow;
end
