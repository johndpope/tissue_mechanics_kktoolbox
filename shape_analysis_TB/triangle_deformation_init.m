function [DM, invDM, F_areas_o] = triangle_deformation_init(C, u, v, w)
nfaces = length(C);
...
R = R - speye(4 * nfaces);
V1pr = double(reshape([V1 ones(length(V1),1)]',length(V1)*4,1));
V2pr = double(reshape([V2 ones(length(V2),1)]',length(V2)*4,1));
V3pr = double(reshape([V3 ones(length(V3),1)]',length(V3)*4,1));
