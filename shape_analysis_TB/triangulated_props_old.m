function [A, V, v, F_areas, h, H, Eb, da] = triangulated_props(X, C, plotflag)
...
if nargout > 4,
    ...
    for ix = 1:length(X),   %loop over the vertices
        ...
        if dA(ix)==0,M(ix) = 0;warning('error in area calculation II');
        else
            H(ix) = H(ix)/dA(ix);
 ...
 disp([A V v h Eb dA da]);
    ...
    if plotflag
        ...
        camlight left;camlight right; view(3);lighting phong;
        light; light;
        str = sprintf('Area: %.2f   Volume: %.2f   v: %.2f   da:%.2f',A, V, v, da);title(str);
        daspect([1 1 1]);drawnow;rotate3d;
%         daspect([1 1 1]);drawnow;rotate3d;
    end
else
    if plotflag
        patch('Vertices', X, 'Faces', C, 'FaceColor', 'red');camlight left;camlight right; view(3);lighting phong;
        light; light;
