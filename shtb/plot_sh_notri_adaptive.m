...
if use_explicit,
    ...
    [Y_LK Y_LK_phi Y_LK_theta Y_PP Y_TT Y_TP] = ylk_explicit_L2(t,p);
        ...
        [t wt]                  = gaussquad(gdim, 0.1, pi);
        [p wp]                  = gaussquad(gdim,0.1,2*pi);
        [p t]                   = meshgrid(p,t);
        [wp wt]                 = meshgrid(wp, wt);
        [Y_LK P_LK]				= precalc_ylk_cos_sin_ng(p, t, L_max);
        Y_LK_phi 				= precalc_ylk_cos_sin_dphi_ng(p, t, L_max, P_LK);
        [Y_LK_theta P_LK_T] 	= precalc_ylk_cos_sin_dtheta_ng(p, t, L_max, P_LK);
        Y_PP 					= precalc_ylk_cos_sin_dphiphi_ng(p, t, L_max, P_LK);
        Y_TT 					= precalc_ylk_cos_sin_dthetatheta_ng(p, t, L_max, P_LK, P_LK_T);
        Y_TP 					= precalc_ylk_cos_sin_dthetaphi_ng(p, t, L_max, P_LK_T);
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if plotflag,
    % %     if sign(1/3.*sum(sum(wp.*wt.*v.*SS)))==-1, H = -H;end
          minH = -0.7 ;  H(H<minH) = minH;
          maxH = 0.2 ;  H(H>maxH) = maxH;
    surf(x,y,z,double(H), 'EdgeColor', 'none');axis equal;axis tight; lighting gouraud;light;
end
