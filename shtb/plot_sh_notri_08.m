...
% dt = 1e-1;t(t>-dt & t<dt)= dt;t(t<pi+dt & t>pi-dt) = pi-1e-6;
% dp = 5e-1;p(p>-dp & p<dp)= dp;p(p<pi+dp & p>pi-dp) = pi-1e-6;
...
% % dfig(ix);
% % disp(ix);
% % clf;surf(p,t,Y_TP(:,:,ix), 'EdgeColor','none');view(3);
% % end
...
% % SS1t = yt.*ztp + ytt.*zp  - ztt.*yp - zt.*ytp;
% % SS2t = ztt.*xp + zt.*xtp  - xt.*ztp - xtt.*zp ;
% % SS3t = xt.*ytp + xtt.*yp  - ytt.*xp - yt.*xtp;
% % 
% % dtSSn = -1./2.*((SS(:,1).^2 + SS(:,2).^2+SS(:,3).^2)).^(-3/2).*...
% %         2.*(SS(:,1).*SS1t(:) + SS(:,2).*SS2t(:) +SS(:,3).*SS3t(:));
% % 
% % nt = [SS1t(:) SS2t(:) SS3t(:)]./SSn(:,ones(1,3)) + ...
% %      SS.* dtSSn(:,ones(1,3)); %%% check this one 
% % 
% % 
% % SS1p = yt.*zpp + ytp.*zp - ztp.*yp - zt.*ypp;
% % SS2p = ztp.*xp + zt.*xpp - xt.*zpp - xtp.*zp;
% % SS3p = xt.*ypp + xtp.*yp - ytp.*xp - yt.*xpp;
% % 
% % dpSSn = -1/2.*((SS(:,1).^2 + SS(:,2).^2+SS(:,3).^2)).^(-3/2).* ...
% %        2.*(SS(:,1).*SS1p(:) + SS(:,2).*SS2p(:) +SS(:,3).*SS3p(:));
% % np = [SS1p(:) SS2p(:) SS3p(:)]./SSn(:,ones(1,3)) + ...
% %     SS.* dpSSn(:, ones(1,3)); %%% check this one 
% % 
% % L = -dot(Xt,nt,2);
