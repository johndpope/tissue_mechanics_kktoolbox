...
A  = zeros(N, M);
for S = 1:length(L)
    A(:, S) = ylk_cos_sin_cs(L(S),K(S),phi, theta)';
end
clks = inv(A'*A)*(A'*R');
...
if flag
    ...
    %surf(x,y,z,-(Rp.^2 + Rt.^2));grid off;daspect([1 1 1]);hold off;rotate3d;drawnow;%colorbar;
    surf(x,y,z,H);grid off;daspect([1 1 1]);hold off;rotate3d;drawnow;%colorbar;
