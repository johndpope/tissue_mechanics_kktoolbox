...
if L_max <0 || L_max > 90, warning('L_max may be out of bounds');disp('L_max');disp(L_max);end
str = sprintf('%s/NOCS_gdim_%d_lmax_%d.mat', precalc_path, length(t), L_max);
...
%         if ~mod(S,50),disp([num2str(S) ' of ' num2str(length(L))]);end;
        Y_LK(:,S) = ylk_cos_sin(L(S),K(S),p',t')'; % usese nocs version
