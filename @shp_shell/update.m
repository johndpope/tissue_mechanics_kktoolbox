...
if obj.needs_updating
    ...
    KG = (L.*N - M.*M)./(E.*G-F.*F);
    %% deformation gradient tensor FT
    
    %% other interesting quantities that we get practically for free
    obj.h = 1./obj.A.*sum(H(:).*obj.basis.w.*SSn);                % total mean curvature
    obj.T = sum(sum(KG(:).*obj.basis.w.*SSn))/4/pi;            % total Gaussian curvature (constant for topology)
    %obj.S = (2.*obj.H.^2-obj.KG).^(0.5);                     % curvedness
    obj.Eb = 1/2*sum((2.*H(:)).^2.*obj.basis.w.*SSn)./8/pi;       % bending energy relative to that of the sphere
