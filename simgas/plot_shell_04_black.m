function plot_shell_04_black(X, F, d, yval)
...
%% define the cut-mesh surface (using the contour indices)
clf;
view(-180,0);
for ix = 1:length(c_ix),  % loop through the connected pairs
    ...
    for lix = 1:length(linx)
        ...
        if v1(2)==yval,  p3 = v1; p4  = v2;
            ...
            p = [p1;p3;p4;p2];patch(p(:,1), p(:,2), p(:,3),'w','EdgeColor','none');
        end
    end
    
end
%%
patch('vertices',X_in,'faces',F,'facecolor','green','edgecolor',[0.3 0.3 0.3]);
patch('vertices',X_out,'faces',F,'facecolor','green','edgecolor','none', 'FaceAlpha', 0.6);
%patch('vertices',Xs,'faces',Fs,'facecolor','green','edgecolor','none','FaceAlpha',0.5);
axis tight;axis off;axis equal;
view(-155,0);lighting phong;camlight
cameramenu
