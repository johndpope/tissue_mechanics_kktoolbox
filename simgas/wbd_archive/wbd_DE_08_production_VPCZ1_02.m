...
counter = 0;
runName = sprintf('DE_08_VPCZ1_02_%2f',rand);
%% Step 0: configure numerical method for shape and gef representation
L_max = 6;           % maximum order of the spherical harmonics parameterization
L_max_gef = 5;
gdim = 30;
optim_method = 2;
pos = [ 0 0 0];
...
%load X_o_dros_embryo_04_ind1.mat;X_o = shp_surface.tr(X_o, L_max);
s_slave.X_o = X_o;
...
fac = gsnl(:).^(1).*gtw(:).^(0) .*ghkb(:).^(0);
phys.C_o = H+lambda_ge*fac/max(fac(:));
...
elseif optim_method == 2
    ...
    mcmc_options.sigQsat         = 1e-2;       % case when constraints are satisfied
    mcmc_options.sigE            = 5.0e-4;       % standard deviation of the energy
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% END of MCMC configuration
        newton_options =   optimset('plotfcn',@DG_opt_plot,...
        'UseParallel','never',...
        'MaxFunEvals', 1e6,...
        'MaxIter', 1e4,...
        'DiffMaxChange', 1e1,...
    ...
    [X_o_res_vec, E_vec, exitflag_vec] = combi_optim_02(X_o,phys,s_master,pos, Y_LK_vm, mcmc_options,newton_options, model_fun, constraints_fun);
     
end
str = sprintf('save %s.mat L_max gdim X_o_res_vec X_o_reference s_master phys pos Y_LK_vm exitflag_vec;',runName);
eval(str);
