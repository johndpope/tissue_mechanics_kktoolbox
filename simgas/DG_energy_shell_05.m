...
omega = kk_dot(tt, tp);
b5 = omega;
%% TESTING!! Calculate midplane lagrange strain tensor as in http://www.scribd.com/doc/27473029/5/Strain-in-Orthogonal-Curvilinear-Coordinates
...
for ix = 1:length(Xt)
    ...
    tol = 1e-4;indx = find(eFTF(1,:)<(1+tol) & eFTF(1,:)>(1-tol)); eFTF(indx) = [];
...
%% TESTING!! calculate Strain tensor components from first term of F in AMoS: p 665 
%%%  using loops
lambda = zeros(length(Xt),2);
...
%           kron(m_l3(ix,:), phys.m_u2(ix,:));
    ...
    tol = 1e-4;indx = find(Eig_vec(1,:)<(tol) & Eig_vec(1,:)>(-tol)); Eig_vec(indx) = [];
    E1 = Eig_vec(1);
    E2 = Eig_vec(2);
...
E_stretch2 = phys.k_stretch/2 * ...
    sum((ai.*ai + phys.a3 * ai .*ai.*ai +phys.a4 * ai.*ai.*ai.*ai).*phys.SSn_o.*w) ;%% multiplied by undeformed area
%% TESTING!! first term of DGT: calculated Cauchy-Green deformation tensor -> principal stretches
...
FT  = kk_transpose(DGT1+DGT3);
C = kk_mx_mult(FT,DGT1+DGT3);% right Cauchy-Green deformation tensor
[res] = real(eig3(reshape(C',3,3,size(C,1)))); % uses vectorized Cardan's formula for root finding
res = sort(res',2);
tol = 1e-4;indx = find(res(1,:)<(1+tol) & res(1,:)>(1-tol));res(:,indx) = [];
...
Eb2 =  1/2*sum(sum(BC,2).^2.*w.*SSn)./8/pi;   % reduced bending energy
disp(['---------------- Bending energy -----------']);
