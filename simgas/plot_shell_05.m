function plot_shell_05(X, F, d, yval,centers,cflag)
% the usual X and C, and the thickness d. val is the y value of the xz cutting plane
%% % note: if the opposite of y is required then uncomment below
%%X(:,2) = -X(:,2);if ~isempty(centers), centers(:,2) = -centers(:,2);end

if nargin<5, centers = [];cflag = 1;end
Fs = F;
if cflag == 1,
    colordef black;
...
%%
if cflag == 1,
    patch('vertices',X_out,'faces',F,'facecolor','green','edgecolor',[0.2 0.2 0.2], 'FaceAlpha', 0.3);
    patch('vertices',X_in,'faces',F,'facecolor','green','edgecolor','none', 'FaceAlpha', 0.5);
    %patch('vertices',Xs,'faces',Fs,'facecolor','green','edgecolor','none','FaceAlpha',0.5);
else
    patch('vertices',X_in,'faces',F,'facecolor','green','edgecolor','none', 'FaceAlpha', 0.7);
    patch('vertices',X_out,'faces',F,'facecolor','green','edgecolor','none', 'FaceAlpha', 0.7);
end
