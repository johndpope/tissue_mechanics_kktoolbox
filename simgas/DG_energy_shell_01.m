...
%% tests that g = dyadic(m_li, m_ui);
Gt1 = [g_l11 g_l12 g_l13 g_l12 g_l22 g_l23 g_l13 g_l23 g_l33];
Gt2 = [g_u11 g_u12 g_u13 g_u12 g_u22 g_u23 g_u13 g_u23 g_u33];
Gt3 = kk_kron(m_l1,m_u1)+kk_kron(m_l2,m_u2)+kk_kron(m_l3,m_u3);
...
%% calculate in-plane shear strain as in AoPaS page 214
omega = kk_dot(tt, tp);
b5 = omega;

%% % % %     %%% Calculate midplane lagrange strain tensor as in AMoS page 667
% beta = zeros(length(Xt),1);
lambda = zeros(length(Xt),2);
% gamma11 = 1/2*(g_l11-phys.g_l11);
% gamma12 = 1/2*(g_l12-phys.g_l12);
% gamma21 = 1/2*(g_l12-phys.g_l12);
% gamma22 = 1/2*(g_l22-phys.g_l22);
...
% eps_vec = [gamma11 gamma12 gamma13 gamma21 gamma22 gamma23 gamma31 gamma32 gamma33];
 eps_vec = [gamma11 gamma12 gamma13 gamma21 gamma22 gamma23 gamma31 gamma32 gamma33];


%  eps_vec = ...                            %% gamma in AMoS
%     gamma11(:,ones(1,9)).*kk_kron(m_u1, m_u1) + ...
%     gamma12(:,ones(1,9)).*kk_kron(m_u1, m_u2) + ...
%     gamma21(:,ones(1,9)).*kk_kron(m_u2, m_u1) + ...
%     gamma22(:,ones(1,9)).*kk_kron(m_u2, m_u2);% + ...
%     gamma13(:,ones(1,9)).*kk_kron(m_u1, m_u3) + ...
%     gamma23(:,ones(1,9)).*kk_kron(m_u2, m_u3) + ...
%     gamma31(:,ones(1,9)).*kk_kron(m_u3, m_u1) + ...
%     gamma32(:,ones(1,9)).*kk_kron(m_u3, m_u2)+...
%     gamma33(:,ones(1,9)).*kk_kron(m_u3, m_u3);
...
%     
%     Eig_vec = eig(C, 'nobalance');
%     E1 = Eig_vec(1);
%     E2 = Eig_vec(2);
%     beta(ix) = (E1 + E2 - ai(ix))./(1 + ai(ix));    % shear component
end
b4 = (lambda(:,1)-lambda(:,2)).^2./2./lambda(:,1)./lambda(:,2);
...
%% % calculate Strain tensor components using loops --- works when no shear
lambda = zeros(length(Xt),2);
beta = zeros(length(Xt),1);
...
disp('Stretch energy');
disp([E_stretch1 E_stretch2 E_stretch3 E_stretch4]);
...
disp('shear');disp([E_shear1 E_shear2 E_shear3 E_shear4 E_shear5 E_shear6]);
%% calculate deformation gradient tensor as AMoS p665 and energy as Munoz
% % % zz = zeros(length(G),1);
% % % on = ones(length(G), 1);
% % % %%% define the metric tensors as the addition of three outer products
% % % g_un = kk_kron(phys.m_l1,phys.m_u1) + kk_kron(phys.m_l2,phys.m_u2) + kk_kron(phys.m_l3, phys.m_u3);
% % % g_de = kk_kron(m_l1,m_u1) + kk_kron(m_l2,m_u2) + kk_kron(m_l3, m_u3);
% % % 
% % % %%%
% % % k_un = [(phys.L_o.*phys.G_o - phys.M_o.*phys.F_o) (phys.M_o.*phys.E_o -phys.L_o.*phys.F_o) zz (phys.M_o.*phys.G_o-phys.N_o.*phys.F_o) (phys.N_o.*phys.E_o-phys.M_o.*phys.F_o) zz zz zz on]./phys.SSn_o(:,ones(1,9));
% % % k_de = [(L.*G - M.*F) (M.*E -L.*F) zz (M.*G-N.*F) (N.*E-M.*F) zz zz zz on]./SSn(:,ones(1,9));
% % % 
% % % %%% calculate the DGT terms; each term is an nx9 array
% % % DGT1 = g_de + phys.D .*k_de;% the first term in the definition of the DGT
% % % DGT2 = kk_kron(m_l1, phys.m_u1) + kk_kron(m_l2, phys.m_u2);
% % % DGT3 = g_un - phys.D .*k_un;% approximation of the third term in the definition of the DGT
% % % DGT = kk_mx_mult(DGT2, DGT3);
% % % DGT = kk_mx_mult(DGT1, DGT);
% % % 
% % % detF = kk_mx_det(DGT);
% % % trFTF = kk_trace(kk_mx_mult(kk_transpose(DGT), DGT));
% % % E = phys.lambda/2*log(detF.^2) + phys.miu/2.*(trFTF-3)-phys.miu/2.*real(log(detF));
% % % E(isnan(E)) = 0;
% % % 
% % % E = sum(E.*w.*SSn);
...
% % X = phys.Y_LK_self(:,1:length(xclks))* [xclks(:) yclks(:) zclks(:)];
% % [E_shear_tri E1_tri E2_tri beta_tri] = shear_calc(X,phys.C_slave,phys.pre,phys.b1, phys.b2, phys.k_shear);
% % disp([E_shear E_shear_tri]);
% % 
