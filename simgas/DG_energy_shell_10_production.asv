function [E, A, V, v_red, AreaDiff, Eb, EADE, E_stretch, E_shear, T, H, phys] = ...
    DG_energy_shell_10_production(X_o,phys)
%% process input
Eb = 0;
EADE = 0;
E_stretch = 0;
...
switch phys.model
    case 'neo Hookean'%% strain energy generalized neo-Hookean solid (AMoS p100)
        %%
%         DGT1 = kk_kron(m_l1, phys.m_u1) + kk_kron(m_l2, phys.m_u2) + kk_kron(n, phys.m_u3);
%         B = kk_mx_mult(DGT1,FT);
%         J = kk_mx_det(B);%
        J = lambda(:,1).*lambda(:,2);%
        I1_bar = (lambda(:,1).^2 + lambda(:,2).^2 + 1)./J.^(2/3); % J = 1 for incompressible materials
%         I1_bar = kk_trace(B)./J.^(2/3);
        PHI = phys.D.*(phys.miu/2*(I1_bar-3) + phys.bulk /2*(J-1).^2);
        E_nHook = sum(PHI.*phys.SSn_o.*w) ;
        Eb      = phys.kb* sum((2.*H(:)-2.*C_o(:)).^2.*w.*SSn);   % calculate bending energy as in RBC paper
        E = E_nHook + Eb;
    case 'Lim'  %% Strain energy Lim et al. 2002
        ...
        E_stretch_lim   = phys.D * phys.k_stretch/2 * sum((ai.*ai + phys.a3 * ai .*ai.*ai +phys.a4 * ai.*ai.*ai.*ai).*phys.SSn_o.*w) ;
        E_shear_lim     = phys.D * phys.k_shear *sum((beta + phys.b1 .* ai .*beta + phys.b2 .* beta.*beta).*phys.SSn_o.*w) ;
        Eb              = phys.kb* sum((2.*H(:)-2.*C_o(:)).^2.*w.*SSn);   % calculate bending energy as in RBC paper
