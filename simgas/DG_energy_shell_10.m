...
%% %%%Calculate shear energy Lim et al 2002
ai = (SSn./phys.SSn_o - 1);     % area_invariant
...
Eb1 = 1/2*sum((2.*H(:)-2.*C_o(:)).^2.*w.*SSn);   % reduced bending energy
...
Eb2 =  sum(sum(BC,2).*w.*SSn);   % bending energy
...
PHI = phys.miu/2.*(trFTF-3) + phys.lambda/2*(detF-1).^2 ; % Strain energy generalized neo-Hookean AMoS
% PHI = phys.miu/2.*(trFTF-3) + phys.lambda/2*log(detF.^2) + -phys.miu/2.*log(detF); % Strain energy as in Conte et al.2007
E_nHook = sum(PHI.*w.*SSn);     % perform the integration
%%
% Eb = phys.D^3/2*sum((2.*H(:)-2.*C_o(:)).^2.*w.*SSn)./8/pi;   % reduced bending energy
Eb = phys.D^3/2*sum((2.*H(:)-2.*C_o(:)).^2.*w.*SSn);   % reduced bending energy
E_Lim = Eb + phys.D*E_stretch5 + phys.D*E_shear3;   % as in Lim 2002 PNAS
%% Reporting II
disp(['----------------  energy -----------']);
disp(['H-C_o (old method): ' num2str(Eb1)]);
disp(['From term2 in F AMoS p665: ' num2str(Eb2)]);
disp('Integrated Gaussian curvature/4/pi');disp(sprintf('%.8f',(T)));

disp(['Energy nHook: ' num2str(E_nHook)]);
disp(['Energy Lim: ' num2str(E_Lim)]);
E = E_nHook;
%% --------------------------------------------------------------------
