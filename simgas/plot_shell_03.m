...
for ix = 1:length(c_ix),  % loop through the connected pairs
    p1 = X_in(c_ix(ix),:);
    p2 = X_out(c_ix(ix),:);
    plot3([p1(1) p2(1)], [p1(2) p2(2)], [p1(3) p2(3)],'b-');hold on;
    linx = L{c_ix(ix)};
    for lix = 1:length(linx)
        ...
        if v1(2)==yval,  p3 = v1; p4  = v2;
            p = [p1;p2;p3;p4];
            patch(p(:,1), p(:,2), p(:,3),'b');%x = [1 1 3 2]';y = [1 3 3 1]';z = ones(size(y));
        end
    end
...
%%
function [X F] = connect_inner_and_outer_shells(X1,X2,F2)
