...
if any(v3d_action)
    ...
    for cix = 1:S{1}.ncameras
        for ix = 1:length(v3d_action)
            if ix<10, chnstr = sprintf('0%d',ix-1);else chnstr = sprintf('%d',ix-1);end
            str = sprintf('*CM%d_CHN%s*', cix-1,chnstr);
            files = dir([fnpath '\' str '.tif']);
            for fix = 1:length(files)
                fprintf(fid,'%s\\%s\n',fnpath,files(fix).name);
            end
        end
    end
    %%
