...
contrast_slice = 64;
file_struct = dir(['Y:\Shared\Processing\Ultrascope 1\11-01-10\Drosophila_Embryo_Histone-eGFP_10x_2µm_Bidirectional_DualCamera_TL_35s.fused\' '*CM1*.tif']);
source_fn = {};
target_fn = {};
for ix = 1:numel(file_struct),
    source_fn{ix} = file_struct(ix).name;
    target_fn{ix} = ['Y:\\Shared\\Processing\\Ultrascope 1\\11-01-10\\CM1\\CM1_out_' num2str(ix) '.tif'];
end
...
for ix = 1:numel(file_struct)
    ...
    fprintf(fid,'makeRectangle(280, 724,1560, 614);run("Crop");\n');
