...
fuse_out = 'fused';
data_root = 'Z:\Ultrascope 1\11-01-13';
exp_tag = 'Drosophila_Larval_CNS_23GO7_RFP_10x_2µm_Bidirectional';
prefix = 'Drosophila_Larval_CNS_23GO7_RFP_10x_2µm_Bidirectional-20110113-132304';
ntime = 1;
nz    = 226;
...
for tix = 0:ntime-1,   % loop over time points
    %% generate TM string
    ...
    if      tix<10,TMp = sprintf('TM-000%d',tix+1);
    elseif  tix<100,TMp = sprintf('TM-00%d',tix+1);
    elseif  tix<1000,TMp = sprintf('TM-0%d',tix+1);
    elseif  tix<10000,TMp = sprintf('TM-%d',tix+1);
    end
    ...
    for cmix = 1:numel(CM)
        ...
        for imix = 1:numel(CHN),
            fn = sprintf('%s\\%s\\%s\\%s\\%s\\%s-%s_%s_%s_%s_%s_PH0_%s.tif', data_root, exp_tag,SPM,TMp,ANG, prefix,SPC, TM, ANG,CM{cmix},CHN{imix}, 'PLN0000');
            %im_info = imfinfo(fn);
            ...
            for zix = 0:nz-1,
                ...
                I_path = sprintf('%s\\%s\\%s\\%s\\%s\\%s-%s_%s_%s_%s_%s_PH0_%s.tif', data_root, exp_tag,SPM,TMp,ANG, prefix,SPC, TM, ANG,CM{cmix},CHN{imix}, PLN);
        ...
        out_fn = 'kk.tif';
        write_tif_stack(uint16(mat2gray(I_fused)*65535),out_fn);
    end
end
