...
%% input Co is in same units as H
%% NOTE: USE bending_energy_prep before calling this function
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%
% M = [];
% dA_vec = [];
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% only px vertex is changing and the ones it is directly lined to
Links = [Links px];
for iter = 1:numel(Links)
    ix = Links(iter);
    V_a = X(ix,:);
...
%         h = sum(real(-M))./A;
Mo = Co.*ddA;
Eb  = 2* sum((M-Mo).^2./ddA)/8/pi;   %bending energy as deviation from preferred curvature
%         dA_vec = ddA;
%         r = sqrt(A/4/pi);
%         dA  = 2 *sum(-M);
%         dAo = 8 * pi * r;
