...
if size(face,2)==4,
    if size(vertex,2)==3,
        X = ones(size(vertex,1), 7) * 255;
        X(:,1:3) = vertex;
        vertex = X;
    end
    plot_mesh_coff_quad(vertex,face);
    return;
end
...
if isempty(face_vertex_color)
     patch('vertices',vertex,'faces',face,'facecolor','green','edgecolor',[edge_color edge_color edge_color]);
    ...
    patch('vertices',vertex,'faces',face,'FaceColor','green','edgecolor', 'none');
end
% colormap gray(256);
lighting flat;
camlight infinite; 
...
function plot_mesh_coff_quad(X,F)
    clf;patch(  'vertices',X(:,1:3),'faces',F,...
            'FaceColor','interp', 'FaceVertexCData',X(:,4:6),...
            'CDataMapping','direct','AlphaDataMapping', 'scaled',...
            'FaceVertexAlphaData', X(:,7),...
            'EdgeColor','k');
%colormap gray(256);
lighting phong;
camlight infinite; 
