function [allX allF] = write_shps_2_separate_stl(S,gdim, fn)
...
allX = [X];
    allF = [C];
str1 = sprintf('%s_%d.raw',fn, ix);str2 = sprintf('%s_%d.stl',fn, ix);mesh2raw(str1,allX,allF);Raw2Stl(str1, str2);disp('Exported stl format');
    counter = counter+1;
end
...
% str = sprintf('%s.ply',fn);write_ply(str, allX, allF);disp('Exported ply format');
